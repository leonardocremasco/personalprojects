...
typedef struct no_ld { // no_ld = nó da lista dupla
	int dado;
	struct no_ld *prox, *ant;
} Tno_ld; // Tno_ld = Tipo de struct do nó da lista dupla
...
{
    int min, max, i;
    clock_t t_ini, t_fim=0, t_fim_anterior;
	int info;
		...
		switch(q) {
            ...
            case 11:
                    ...
                    for (i=1; i<=100000; i++)
                    {
                        ...
                        Inserir_fim_LD (&ini, i);
                        if ((i%1000)==0)
                        {
                            t_fim_anterior = t_fim;
                            t_fim = clock() - t_ini;
                            printf("Tempo de execucao: %0.5lf ms\n", (double)(t_fim-t_fim_anterior)*1000/CLOCKS_PER_SEC);
                        }
                    }
                    system("pause");
					break;
